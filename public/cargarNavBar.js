...
async function cargarNavbar(usuario) {
    ...
    function XhablarTexto(texto, item) {
        detenerAudio();
        ...
        utteranceActual.onerror = () => detenerAudio();
    ...
    function XXhablarTexto(texto, item) {
        detenerAudio();
        if (!('speechSynthesis' in window)) return;

        // 🔹 Esperamos a tener la voz lista antes de hablar
        obtenerVozEspañol((voz) => {
            ...
            // 🧩 IMPORTANTE: cancelar cualquier cola previa
            speechSynthesis.cancel();
            // 🕐 Pequeño delay evita el bug del “corte prematuro”
            setTimeout(() => speechSynthesis.speak(utteranceActual), 200);
        });
    }
